...
must_step "build.base_worker_images" docker compose -f "${LOCAL_TEST_COMPOSE}" build base-runtime worker
...
must_step "register.apply_deployment" env PREFECT_API_URL="${PREFECT_API_URL}" PREFECT_CLIENT_CUSTOM_HEADERS="${PREFECT_CUSTOM_HEADERS_JSON}" PREFECT_WORK_POOL="${PREFECT_WORK_POOL}" PREFECT_WORK_QUEUE="${PREFECT_WORK_QUEUE}" uv run python "${ROOT_DIR}/scripts/register/prefect_apply.py" --flow-source "${REGISTER_FLOW_SOURCE}" --flow-entrypoint "${REGISTER_FLOW_ENTRYPOINT}" --fixed-name "${REGISTER_FIXED_DEPLOYMENT_NAME}" --colab-name "${REGISTER_COLAB_DEPLOYMENT_NAME}"
