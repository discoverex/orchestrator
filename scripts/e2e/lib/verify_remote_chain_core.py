...
def run_ops_script(script_rel_path: str, env_overrides: dict[str, str], argv: list[str]) -> dict[str, Any]:
...
def command_wait_completed(args: Any, http_json_fn: Callable[..., Any]) -> dict[str, Any]:
    ...
    while time.time() < deadline:
        out = http_json_fn("GET", f"{args.prefect_api_url.rstrip('/')}/flow_runs/{args.flow_run_id}", headers=headers)
        if not isinstance(out, dict):
            raise VerifyError("prefect-wait-completed", "PREFECT_BAD_RESPONSE", "expected dict response")
        state_type = (out.get("state_type") or out.get("state", {}).get("type") or "").upper()
...
def command_storage_objects(args: Any, http_json_fn: Callable[..., Any]) -> dict[str, Any]:
    ...
    for object_uri in uris.values():
        out = http_json_fn("POST", f"{base}/v1/object/head", payload={"object_uri": object_uri}, headers=headers)
...
def command_flush_verify(args: Any, run_ops_script_fn: Callable[..., Any]) -> dict[str, Any]:
    env = {
...
def command_prune_verify(args: Any, run_ops_script_fn: Callable[..., Any], http_json_fn: Callable[..., Any]) -> dict[str, Any]:
