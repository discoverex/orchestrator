...
def build_router(app) -> APIRouter:
    ...
    def to_response(req: PresignRequest, method: str, request: Request) -> PresignResponse:
        try:
    ...
    @router.post("/v1/presign/put", response_model=PresignResponse, dependencies=[Depends(authorize)])
    def presign_put(req: PresignRequest, request: Request) -> PresignResponse:
        return to_response(req, method="PUT", request=request)

    @router.post("/v1/presign/get", response_model=PresignResponse, dependencies=[Depends(authorize)])
    def presign_get(req: PresignRequest, request: Request) -> PresignResponse:
        return to_response(req, method="GET", request=request)

    @router.post("/v1/presign/batch", response_model=list[PresignResponse], dependencies=[Depends(authorize)])
    ...
    @router.post("/v1/object/head", response_model=HeadResponse, dependencies=[Depends(authorize)])
